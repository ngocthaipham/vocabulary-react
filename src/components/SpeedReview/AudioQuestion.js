...
const AudioQuestion = (props) => {
  ...
  const data = [
    {
      questions: (
        <div>
        <label>
          <audio style={{display: 'none'}} src={`http://localhost:5000/audios/${audioQuestions[currentQuestion]}`} autoPlay></audio>
          <img
            onClick={() => {
              var audio = new Audio(
                `http://localhost:5000/audios/${audioQuestions[currentQuestion]}`
              );
              audio.play();
            }}
  ...
  useEffect(() => {
    Axios.get(`http://localhost:5000/vocabs/${idLevel}`).then((response) => {
      setAudioQuestions(response.data.result.map((question) => question.audioWord));
