...
const WordTable = () => {
  ...
  return (
    <>
<button id='add-vocab-btn' onClick={() => {showAddVocabForm()}}>Add a new course</button>
    <br/>
    <div id="add-vocab-form" style={{display: 'none'}}> 
      <h3>Add vocab</h3>
        <form onSubmit={(e)=>{
            ...
            <label>ID</label>
            <input type="number" name="id-vocab"  onChange={(e) => {
            ...
            <label>Meaning</label>
            <input type="text" name="meaning"  onChange={(e) => {
                setMeaning(e.target.value) 
            }} />
            <button type="submit" onClick={() =>{cancelAddVocabForm()}}>Add new vocab</button>
            <button type="button" onClick={() => {cancelAddVocabForm()}}>Cancel</button>
        </form>
        </div>
        <br/>
        ...
        onSubmit={e =>{
            ...
            <label>ID</label>
            <input type="number" id="id-vocab-input" onChange={(e) => {
                setId(e.target.value)
            }} />
            <label>ID Level</label>
            <input type="number" name="id-level"  onChange={(e) => {
    ...
    <button type="submit" onClick={()=>{cancelUpdateVocabForm()}}>Save</button> 
    <button type="button" onClick={() =>{cancelUpdateVocabForm()}}>Cancel</button>
        </form>
</div>
    ...
    <table border="5" cellPadding ="5" >
      ...
      <tbody>
          {vocabList.map(word => (
              ...
              <td>{word.meaning}</td>
          <td>
            <button onClick={() =>{showUpdateVocabForm(word.id)}}
